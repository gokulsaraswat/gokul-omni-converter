...
class BuildCenterWindow(tk.Toplevel):
    def __init__(self, app: "GokulOmniConvertLiteApp", palette: ThemePalette) -> None:
        ...
        ttk.Label(
            header,
            text=(
                "Patch 9 adds exportable diagnostics, state snapshots, richer installer assets, and direct entry points into the build folder so the app can move toward a packaged release more cleanly."
            ),
            style="CardBody.TLabel",
            wraplength=620,
            justify="left",
...
class AboutProfileEditorWindow(tk.Toplevel):
    def __init__(self, app: "GokulOmniConvertLiteApp", palette: ThemePalette) -> None:
        ...
        ttk.Label(
            header,
            text="Update your image path, bio, and social links here. Saving writes back to about_profile.json and refreshes the About page immediately.",
            style="CardBody.TLabel",
            wraplength=720,
            justify="left",
        ).grid(row=1, column=0, sticky="w", pady=(8, 0))
...
class GokulOmniConvertLiteApp(tk.Tk):
    ...
    def _build_home_page(self) -> None:
        ...
        ttk.Label(
            hero,
            text="Patch 9 keeps the organizer, automation, and pure Python engine, then adds direct SMTP delivery, a richer About editor, diagnostics export, settings snapshots, and stronger installer-prep assets for release work later.",
        ...
        ttk.Button(dep_card, text="Refresh dependencies", command=self._refresh_dependency_status).grid(row=2, column=0, sticky="w", pady=(16, 0))
    ...
    def _build_pdf_tools_page(self) -> None:
        ...
        self.pdf_tool_password_label.grid(row=14, column=0, sticky="w", pady=(10, 0))
        self.pdf_tool_password_entry = ttk.Entry(options, textvariable=self.pdf_tool_password_var, show="•")
        self.pdf_tool_password_entry.grid(row=14, column=1, columnspan=3, sticky="ew", padx=(8, 0), pady=(10, 0))

        self.pdf_tool_owner_password_label = ttk.Label(options, text="Owner password:", style="Surface.TLabel")
        self.pdf_tool_owner_password_label.grid(row=15, column=0, sticky="w", pady=(10, 0))
        self.pdf_tool_owner_password_entry = ttk.Entry(options, textvariable=self.pdf_tool_owner_password_var, show="•")
        self.pdf_tool_owner_password_entry.grid(row=15, column=1, columnspan=3, sticky="ew", padx=(8, 0), pady=(10, 0))

        self.pdf_tool_compression_label = ttk.Label(options, text="Compression profile:", style="Surface.TLabel")
        self.pdf_tool_compression_label.grid(row=16, column=0, sticky="w", pady=(10, 0))
        ...
        self.pdf_tool_compression_combo.grid(row=16, column=1, sticky="w", padx=(8, 0), pady=(10, 0))
        ...
        self.pdf_tool_options_help_label.grid(row=17, column=0, columnspan=4, sticky="w", pady=(12, 0))
    ...
    def _build_ocr_page(self) -> None:
        ...
        ttk.Label(
            ...
            text=(
                ...
                "Patch 11 adds an integrated OCR workspace with Tesseract detection, progress reporting, and history logging."
            ),
            style="CardBody.TLabel",
            wraplength=980,
            justify="left",
    ...
    def _update_pdf_tool_controls(self) -> None:
        ...
        text_enabled = tool in {PDF_TOOL_WATERMARK_TEXT, PDF_TOOL_TEXT_OVERLAY, PDF_TOOL_REDACT_TEXT, PDF_TOOL_SIGN_VISIBLE}
    ...
    def _start_pdf_tool(self) -> None:
        ...
        if tool in {PDF_TOOL_WATERMARK_TEXT, PDF_TOOL_TEXT_OVERLAY, PDF_TOOL_REDACT_TEXT} and not text_value:
    ...
    def _build_history_page(self) -> None:
        ...
        ttk.Button(action_row, text="Clear history", command=self._clear_history).grid(row=2, column=0, sticky="ew", pady=(8, 0))
    ...
    def _build_settings_page(self) -> None:
        ...
        header.grid_columnconfigure(0, weight=1)
        ttk.Label(header, text="Appearance, engine selection, OCR, organizer, and local files", style="CardTitle.TLabel").grid(row=0, column=0, sticky="w")
        ttk.Label(
            header,
            text=(
                "Patch 9 keeps the pure Python path front and center, adds direct SMTP delivery, diagnostics export, settings snapshots, an in-app About editor, and richer installer assets "
                "so the app can move more cleanly toward a packaged release later."
            ),
            style="CardBody.TLabel",
            wraplength=980,
            justify="left",
        ...
        ttk.Label(
            appearance,
            text="Theme, output folder, engine choice, and the soffice path are saved automatically when you close the app.",
            style="CardBody.TLabel",
            wraplength=460,
            justify="left",
        ).grid(row=2, column=0, columnspan=3, sticky="w", pady=(12, 0))
        ...
        ttk.Button(soffice_actions, text="Test", command=self._test_soffice_path).grid(row=0, column=1)
        ...
        ttk.Button(actions_frame, text="Test Tesseract", command=self._test_tesseract_path).grid(row=1, column=0, sticky="ew", pady=(8, 0))
        ttk.Button(actions_frame, text="Open organizer", command=lambda: self._show_page("organizer")).grid(row=2, column=0, sticky="ew", pady=(8, 0))
        ttk.Button(actions_frame, text="Open footer notes", command=self._open_notes_window).grid(row=3, column=0, sticky="ew", pady=(8, 0))
        ttk.Button(actions_frame, text="Open About page", command=self._show_about).grid(row=4, column=0, sticky="ew", pady=(8, 0))
        ttk.Button(actions_frame, text="Edit About profile", command=self._open_about_editor_window).grid(row=5, column=0, sticky="ew", pady=(8, 0))
        ttk.Button(actions_frame, text="Open SMTP delivery", command=self._open_smtp_window).grid(row=6, column=0, sticky="ew", pady=(8, 0))
        ttk.Button(actions_frame, text="Send latest outputs", command=self._send_last_outputs_via_smtp).grid(row=7, column=0, sticky="ew", pady=(8, 0))
        ttk.Button(actions_frame, text="Save EML draft", command=self._save_eml_draft_for_last_outputs).grid(row=8, column=0, sticky="ew", pady=(8, 0))
        ttk.Button(actions_frame, text="Open Build Center", command=self._open_build_center_window).grid(row=9, column=0, sticky="ew", pady=(8, 0))
        ttk.Button(actions_frame, text="Open build notes", command=lambda: open_path(self.build_notes_path)).grid(row=10, column=0, sticky="ew", pady=(8, 0))
        ttk.Button(actions_frame, text="Export settings snapshot", command=self._export_state_snapshot_action).grid(row=11, column=0, sticky="ew", pady=(8, 0))
        ttk.Button(actions_frame, text="Import settings snapshot", command=self._import_state_snapshot_action).grid(row=12, column=0, sticky="ew", pady=(8, 0))
        ttk.Button(actions_frame, text="Export diagnostics report", command=self._export_diagnostics_report_action).grid(row=13, column=0, sticky="ew", pady=(8, 0))
        ttk.Button(actions_frame, text="Reload About profile", command=self._refresh_about_profile).grid(row=14, column=0, sticky="ew", pady=(8, 0))
        ttk.Button(actions_frame, text="Refresh history view", command=self._refresh_history_views).grid(row=15, column=0, sticky="ew", pady=(8, 0))
        ttk.Button(actions_frame, text="Clear local history", command=self._clear_history).grid(row=16, column=0, sticky="ew", pady=(8, 0))
        ttk.Label(actions_frame, textvariable=self.smtp_status_var, style="CardBody.TLabel", wraplength=320, justify="left").grid(row=17, column=0, sticky="w", pady=(12, 0))
    ...
    def _build_about_page(self) -> None:
        ...
        ttk.Label(
            header,
            text="This page reads your editable profile JSON and shows your image, details, and social links. Patch 9 also adds an in-app profile editor, installer-prep shortcuts, and SMTP-ready contact actions so the app is easier to personalize before packaging.",
            style="CardBody.TLabel",
            wraplength=980,
            justify="left",
        ).grid(row=1, column=0, sticky="w", pady=(8, 0))
        ...
        ttk.Button(profile_actions, text="Open SMTP delivery", command=self._open_smtp_window).grid(row=4, column=0, sticky="ew", pady=(8, 0))
        ttk.Button(profile_actions, text="Open Build Center", command=self._open_build_center_window).grid(row=5, column=0, sticky="ew", pady=(8, 0))
        ttk.Button(profile_actions, text="Open build notes", command=lambda: open_path(self.build_notes_path)).grid(row=6, column=0, sticky="ew", pady=(8, 0))
        ...
        self.about_meta_label.grid(row=2, column=0, sticky="w", pady=(12, 0))
        self.about_bio_label = ttk.Label(info_card, style="CardBody.TLabel", wraplength=680, justify="left")
        self.about_bio_label.grid(row=3, column=0, sticky="w", pady=(12, 0))

        ttk.Separator(info_card, orient="horizontal").grid(row=4, column=0, sticky="ew", pady=14)
        ttk.Label(info_card, text="Links", style="CardTitle.TLabel").grid(row=5, column=0, sticky="w")
        self.about_links_frame = ttk.Frame(info_card)
        self.about_links_frame.grid(row=6, column=0, sticky="w", pady=(10, 0))

        ttk.Separator(info_card, orient="horizontal").grid(row=7, column=0, sticky="ew", pady=14)
        ttk.Label(info_card, text=f"Version: {APP_VERSION}", style="CardBody.TLabel").grid(row=8, column=0, sticky="w")
        ttk.Label(
            info_card,
            text="This page is editable through about_profile.json or the new in-app profile editor, so you can later swap the placeholder image, update social links, and prepare the app for installer packaging.",
            style="CardBody.TLabel",
            wraplength=680,
            justify="left",
    ...
    def _refresh_about_profile(self) -> None:
        ...
        if hasattr(self, "about_name_label"):
            ...
            if not active_links:
                ttk.Label(self.about_links_frame, text="No social links configured yet. Use the in-app editor or edit about_profile.json to add them.", style="CardBody.TLabel").grid(row=0, column=0, sticky="w")
            else:
                for index, item in enumerate(active_links):
                    label = str(item.get("label", "Link")).strip() or "Link"
                    url = str(item.get("url", "")).strip()
                    ttk.Button(self.about_links_frame, text=label, command=lambda target=url: open_url(target)).grid(row=0, column=index, padx=(0 if index == 0 else 8, 0), sticky="w")
    ...
    def _update_mode_controls(self) -> None:
        ...
        image_mode = mode in {MODE_PDF_TO_IMAGES, MODE_PDF_TO_PPTX}
    ...
    def _refresh_home_summary(self) -> None:
        ...
        if self.running:
            ...
            self.home_hint_var.set("No inputs are loaded yet. Add files or a folder from Home or Convert.")
        else:
        ...
        try:
            while True:
                ...
                if message_type == "log":
                    ...
                    self.last_outputs = [Path(item) for item in outputs]
                    self.last_output_dir = Path(str(config.output_dir))
                    self.last_job_label = f"PDF Tool -> {config.tool}"
    ...
    def _apply_theme(self, initial: bool = False) -> None:
        ...
        for widget in (self.file_listbox, self.log_text, self.pdf_tool_listbox, self.pdf_tool_log_text, self.ocr_log_text, self.history_details_text, self.automation_log_text):
    ...
    def _build_automation_page(self) -> None:
        ...
        ttk.Label(
            header,
            text=(
                "Patch 8 adds reusable presets, a watch-folder automation workflow, and better ways to bundle or report the latest outputs. "
        ...
        self.preset_tree = ttk.Treeview(
            presets_frame,
            columns=("name", "mode", "engine", "merge"),
            show="headings",
            height=9,
        )
        ...
        self.preset_tree.column("name", width=150, anchor="w")
        self.preset_tree.column("mode", width=200, anchor="w")
        ...
        ttk.Button(preset_actions, text="Delete", command=self._delete_selected_preset).grid(row=0, column=1, sticky="ew", padx=(8, 0))
        ttk.Button(preset_actions, text="Export JSON", command=self._export_presets).grid(row=0, column=2, sticky="ew", padx=(8, 0))
        ttk.Button(preset_actions, text="Import JSON", command=self._import_presets).grid(row=0, column=3, sticky="ew", padx=(8, 0))
    ...
    def _on_preset_selected(self, _event=None) -> None:
        ...
        self.watch_summary_var.set(
            f"Selected preset '{preset.get('name', '')}' -> {preset.get('mode', '')} using {preset.get('engine_mode', ENGINE_AUTO)}."
        )
    ...
    def _refresh_presets_view(self) -> None:
        ...
        for index, preset in enumerate(self.state_store.presets()):
            iid = f"preset_{index}"
            values = (
                preset.get("name", ""),
                preset.get("mode", ""),
    ...
    def _apply_selected_preset(self) -> None:
        ...
        self._update_mode_controls()
        self._show_page("convert")
        self.status_var.set(f"Applied preset '{preset.get('name', '')}'.")
        ...
        try:
            while True:
                ...
                if message_type == "log":
                    ...
                    self.last_output_dir = output_dir
                    self.last_job_label = str(record.get("mode", "OCR"))
    ...
    def _on_close(self) -> None:
        ...
        if self.automation_after_id:
            ...
            self.automation_after_id = None
        self._persist_state()
        for window in (self.notes_window, self.smtp_window, self.build_center_window, self.about_editor_window):
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument("--smoke-test-ui", action="store_true", help="Open the app briefly and exit.")
    args = parser.parse_args(argv)

    app = GokulOmniConvertLiteApp()
